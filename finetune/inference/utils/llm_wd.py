...
class VLLMModels(Enum):
    ...
    LLAMA31_8B_ins = 'meta-llama/Meta-Llama-3.1-8B-Instruct',
    MISTRAL_7B_ins = "Mistral-7B-Instruct-v0.3",
    DEEPSEEK_ins = "deepseek-coder-6.7b-instruct",
    QWEN_ins = "qwen-7b-instruct",
    LLAMA31_8B_pure = 'meta-llama/Meta-Llama-3.1-8B-Pure',
    LLAMA31_8B_num = 'meta-llama/Meta-Llama-3.1-8B-Num',
...
class LLMClient:
    ...
    def _hash_prompt(self, prompt, model, temperature, max_tokens, top_p, cnt, beam_search, best_of):
        # Create a unique hash for the given parameters
        hash_input = f"{prompt}-{model}-{temperature}-{max_tokens}-{self.system_content}-{top_p}--{cnt}--{beam_search}--{best_of}".encode()
        return hashlib.md5(hash_input).hexdigest()

    def send_request(self, prompt, model, temperature, max_tokens, top_p, num_samples, beam_search = False, best_of = 3):
        if beam_search:
            temperature = 0
            response = self.client.chat.completions.create(
                ...
                top_p=top_p,
                n=num_samples,
                extra_body={'use_beam_search': beam_search, "best_of": best_of})
        else:
            response = self.client.chat.completions.create(
                model=model,
                messages=[
    ...
    def get_samples_from_cache(self, prompt, model, temperature, max_tokens, top_p, cnt = 0, beam_search = False, best_of = 3):
        # Create a unique hash for the prompt and parameters (excluding num_samples)
        # cache_key = self._hash_prompt(prompt, model, temperature, max_tokens, top_p, cnt)
        cache_key = self._hash_prompt(prompt, model, temperature, max_tokens, top_p, cnt, beam_search, best_of)
        print(cache_key)
        return self.cache.get(cache_key, [])

    def add_samples_to_cache(self, prompt, model, temperature, max_tokens, top_p, samples, cnt, beam_search, best_of):
        cache_key = self._hash_prompt(prompt, model, temperature, max_tokens, top_p, cnt, beam_search, best_of)
        cached_samples = self.cache.get(cache_key, [])
        cached_samples.extend(samples)
        self.cache[cache_key] = cached_samples

    def generate(self, prompt, num_samples, model=None, temperature=0.7, max_tokens=1024, top_p=1, cnt=0, logger = None, beam_search = False, best_of = 3):
        # model = self.check_model_name(model)
        cached_samples = self.get_samples_from_cache(prompt, model, temperature, max_tokens, top_p, cnt, beam_search, best_of)

        # If the number of cached samples is less than requested, generate more samples
        if len(cached_samples) < num_samples:
            # print(len(cached_samples), num_samples)
            # exit()
            ...
            while not actually_got_samples:
                try:
                    response = self.send_request(prompt, model, temperature, max_tokens, top_p, remaining_samples, beam_search, best_of)
                    # new_samples = [c.message.content for c in response.choices]
                    new_samples = [c for c in response.choices]
                    self.add_samples_to_cache(prompt, model, temperature, max_tokens, top_p, new_samples, cnt, beam_search, best_of)
                    ...
                    if "Rate limit reached for model" in str(e):
                        ...
                        logger.error(f"Error, going to try again in 1 second {e}")
