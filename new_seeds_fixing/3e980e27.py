...
def generate_input():
    ...
    for i in range(number_object):
        # Generate the object pattern need to be reconstructed, use gray to represent the object pattern
        object_pattern = random_sprite(n=3, m=3, color_palette=[Color.GRAY], density=0.5, symmetry="not_symmetric")
        ...
        while object_pattern[position_x, position_y] != Color.GRAY:
            position_x, position_y = np.random.randint(0, 3), np.random.randint(0, 3)
    ...
    grid[grid == Color.GRAY] = Color.BLACK
