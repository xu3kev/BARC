...
def main(input_grid: np.ndarray) -> np.ndarray:
    ...
    for x in range(input_grid.shape[0]):
        for y in range(input_grid.shape[1]):
            if input_grid[x, y] == green:
                # find the closest point on the lines
                closest_x, closest_y = min([(i, j) for i in range(input_grid.shape[0]) for j in range(input_grid.shape[1]) if lines[i, j] != black],
...
def generate_input() -> np.ndarray:
    ...
    line_colors = random.sample([color for color in range(10) if color != green and color != black], 2)
