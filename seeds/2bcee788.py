...
def main(input_grid):
    ...
    if collision(object1=translate(main_object, x=1, y=0), object2=red_pixels):
        ...
        symmetry = MirrorSymmetry(mirror_x=object_position(main_object, anchor="right center")[0]+0.5, mirror_y=None)
    # on the left?
    elif collision(object1=translate(main_object, x=-1, y=0), object2=red_pixels):
        symmetry = MirrorSymmetry(mirror_x=object_position(main_object, anchor="left center")[0]-0.5, mirror_y=None)
    # on the top?
    elif collision(object1=translate(main_object, x=0, y=-1), object2=red_pixels):
        symmetry = MirrorSymmetry(mirror_x=None, mirror_y=object_position(main_object, anchor="top center")[1]-0.5)
    # on the bottom?
    elif collision(object1=translate(main_object, x=0, y=1), object2=red_pixels):
        symmetry = MirrorSymmetry(mirror_x=None, mirror_y=object_position(main_object, anchor="bottom center")[1]+0.5)
    else:
