...
def generate_input():
    ...
    blue_square = random_sprite(2, 2, density=1, color_palette=[Color.BLUE])
    for _ in range(np.random.randint(1,6)):
        # place a blue square on the grid if there is room
        try:
            x, y = random_free_location_for_object(grid, blue_square, padding=1)
    ...
    red_square = random_sprite(2, 2, density=1, color_palette=[Color.RED])
    for _ in range(np.random.randint(1,6)):
        # place a red square on the grid if there is room
        try:
            x, y = random_free_location_for_object(grid, red_square, padding=1)
